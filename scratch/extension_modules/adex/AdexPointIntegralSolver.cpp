...
#include "AdexPointIntegralSolver.h"
...
namespace dolfin {
  ...
  //-----------------------------------------------------------------------------
  AdexPointIntegralSolver::AdexPointIntegralSolver(std::shared_ptr<MultiStageScheme> scheme) :
    Variable("AdexPointIntegralSolver", "unnamed"), _scheme(scheme),
  ...
  //-----------------------------------------------------------------------------
  AdexPointIntegralSolver::~AdexPointIntegralSolver()
  {
    // Do nothing
  }
  //-----------------------------------------------------------------------------
  void AdexPointIntegralSolver::reset_newton_solver()
  {
  ...
  //-----------------------------------------------------------------------------
  void AdexPointIntegralSolver::reset_stage_solutions()
  {
  ...
  //-----------------------------------------------------------------------------
  void AdexPointIntegralSolver::step(double dt)
  {
    dolfin_assert(_mesh);
  
    Timer timer("AdexPointIntegralSolver::step");
    ...
    *_scheme->dt() = dt;
    ...
    try
    {
      ...
      Timer timer_apply("AdexPointIntegralSolver::apply");
      for (unsigned int stage=0; stage<_num_stages; stage++)
  ...
  //-----------------------------------------------------------------------------
  void AdexPointIntegralSolver::_solve_explicit_stage(std::size_t vert_ind,
                                                  unsigned int stage,
  ...
  //-----------------------------------------------------------------------------
  void AdexPointIntegralSolver::_solve_implicit_stage(std::size_t vert_ind,
                                                  unsigned int stage,
                                                  const Cell& cell,
                                                  const ufc::cell& ufc_cell,
                                                  const std::vector<double>& coordinate_dofs)
  {
    // Do a simplified newton solve _simplified_newton_solve(vert_ind, stage, cell, ufc_cell, coordinate_dofs);
  ...
  //-----------------------------------------------------------------------------
  void AdexPointIntegralSolver::step_interval(double t0, double t1, double dt)
  {
    if (dt <= 0.0)
    {
      dolfin_error("AdexPointIntegralSolver.cpp",
                   "stepping AdexPointIntegralSolver",
  ...
  //-----------------------------------------------------------------------------
  void AdexPointIntegralSolver::_compute_jacobian(std::vector<double>& jac,
  ...
  //-----------------------------------------------------------------------------
  void AdexPointIntegralSolver::_lu_factorize(std::vector<double>& A)
  {
    // Local variables
    double sum;
  ...
  //-----------------------------------------------------------------------------
  void AdexPointIntegralSolver::_forward_backward_subst(const std::vector<double>& A,
  ...
  //-----------------------------------------------------------------------------
  double AdexPointIntegralSolver::_norm(const std::vector<double>& vec) const
  {
    double l2_norm = 0;
  ...
  //-----------------------------------------------------------------------------
  void AdexPointIntegralSolver::_check_forms()
  {
    ...
    for (unsigned int i = 0; i < stage_forms.size(); i++)
    {
      for (unsigned int j = 0; j < stage_forms[i].size(); j++)
      {
        ...
        {
          dolfin_error("AdexPointIntegralSolver.cpp",
                       "constructing AdexPointIntegralSolver",
  ...
  //-----------------------------------------------------------------------------
  void AdexPointIntegralSolver::_init()
  {
    dolfin_assert(_mesh);
  ...
  //-----------------------------------------------------------------------------
  void AdexPointIntegralSolver::_simplified_newton_solve(
    ...
    const ufc::cell& ufc_cell, const std::vector<double>& coordinate_dofs)
  {
    ...
    do
    {
      ...
      if (newton_iterations > max_iterations)
      {
        ...
        dolfin_error("AdexPointIntegralSolver.h",
                     "Newton point integral solver",
                     "Newton solver in AdexPointIntegralSolver exceeded maximal iterations");
      }
  ...
  void AdexPointIntegralSolver::_apply_condition(std::vector<double> *last_stage_solution) {
  
      double spike = parameters["spike"];
      double resting_state = parameters["V_T"];
      double adaptation = parameters["b"];

      for (unsigned int row=0; row < _system_size; row++) {
          for (
                  std::vector<double>::iterator it = last_stage_solution[row].begin();
                  it != last_stage_solution[row].end();
                  ++it
              ) {
              if (*it >= spike) {
                  if (row == 1) {
